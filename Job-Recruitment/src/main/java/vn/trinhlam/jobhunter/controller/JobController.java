...
public class JobController {
    ...
    public ResponseEntity<Void> delete(@PathVariable("id") long id) throws IdInvalidException {
        Optional<Job> curreOptional = this.jobService.fetchJobById(id);
        if (!curreOptional.isPresent()) {
            throw new IdInvalidException("Job không tìm thấy");
        }
    ...
    public ResponseEntity<Job> getJob(@PathVariable("id") long id) throws IdInvalidException {
        Optional<Job> curreOptional = this.jobService.fetchJobById(id);
        if (curreOptional.isPresent() == false) {
            throw new IdInvalidException("Job không tìm thấy");
        }
        return ResponseEntity.ok().body(curreOptional.get());
    }
